...
function mapToAnalysisResult(b: BackendResult): AnalysisResult {
  ...
  const records = b.cases.map((c) => {
    ...
    return {
      ...
      bookingRefExtracted: (raw.ext_booking_ref as string) ?? (raw.booking_ref as string) ?? null,
      loadRefExtracted: (raw.ext_load_ref as string) ?? null,
      containerExtracted: (raw.ext_container as string) ?? null,
      mrnRefExtracted: (raw.ext_mrn as string) ?? null,
