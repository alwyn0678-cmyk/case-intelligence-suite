...
// ─── Missing Load Ref Undercount Detection ────────────────────────

/**
 * Checks if a record has explicit "missing load ref" language but primaryIssue
 * is not 'load_ref'. Returns true when the case appears to be a misclassification.
 *
 * Use validateMissingLoadRefUndercount(records) to scan the full dataset.
 *
 * @example
 * isMissingLoadRefUndercount('ref_provided', 'please send the loadref for this order')
 * // → true (explicit missing lang but wrong issue)
 *
 * isMissingLoadRefUndercount('load_ref', 'load ref missing please send')
 * // → false (correctly classified)
 */
const EXPLICIT_MISSING_LOAD_REF_PATTERNS: string[] = [
  ...
  'please send loadref', 'loadref missing', 'no loadref', 'no load reference',
  'laadreferentie ontbreekt', 'loadref ontbreekt', 'graag loadref sturen',
  'graag laadreferentie sturen', 'loadref nog niet ontvangen',
  'laadreferentie niet ontvangen', 'ladereferenz fehlt', 'fehlende ladereferenz',
];

export function isMissingLoadRefUndercount(
  primaryIssue: string,
  combinedText: string,
): boolean {
  if (primaryIssue === 'load_ref') return false;
  ...
  const lower = combinedText.toLowerCase();
  return EXPLICIT_MISSING_LOAD_REF_PATTERNS.some(p => lower.includes(p));
}

/**
 * Scans all records for cases with explicit missing-load-ref language
 * that were NOT classified as load_ref.
 * Returns the list of misclassified case indices.
 */
export function validateMissingLoadRefUndercount(
  records: Array<{ primaryIssue: string; subject?: string | null; description?: string | null; isr_details?: string | null }>,
): number[] {
  const misclassified: number[] = [];
  for (let i = 0; i < records.length; i++) {
    const r = records[i];
    const text = [r.subject, r.description, r.isr_details].filter(Boolean).join(' ');
    if (isMissingLoadRefUndercount(r.primaryIssue, text)) {
...
/**
 * Returns true if the text would trigger financial classification ONLY via
 * the bare "rate" substring — i.e. via words like "generated", "auto-generated",
 * "translated", "rated" where "rate" appears as a substring, not a whole word.
 *
 * This detects false positives in the financial classifier.
 *
 * @example
 * validateFinancialSubstringFalsePositive('rate', 'the email was auto-generated')
 * // → true (contains 'rate' as substring of 'generated', not standalone)
 *
 * validateFinancialSubstringFalsePositive('rate', 'price correction needed on invoice')
 * // → false (legitimate financial trigger)
 */
const RATE_SUBSTRING_FALSE_POSITIVE_WORDS = [
  'generated', 'auto-generated', 'translated', 'generated by', 'rated', 'rating',
  'decorated', 'integrated', 'migrated', 'operated', 'moderated', 'enumerated',
];

const FINANCIAL_STRONG_TRIGGERS: string[] = [
  'rate query', 'rate dispute', 'invoice query', 'overcharge', 'billing query',
  'charge dispute', 'incorrect invoice', 'rate discrepancy', 'invoice incorrect',
  'wrong invoice', 'invoice dispute', 'overcharged', 'undercharged', 'charge query',
  'selfbilling', 'self billing', 'self-billing', 'extra cost invoice',
  'extra costs invoice', 'billing report', 'billing issue', 'billing error',
  'cost invoice', 'demurrage invoice', 'detention invoice', 'storage invoice',
  'credit note', 'debit note', 'price correction', 'price adjustment', 'rate correction',
  'wrong rate applied', 'corrected invoice', 'invoice correction',
  'invoice', 'billing', 'extra cost', 'extra costs', 'additional charge',
];

export function validateFinancialSubstringFalsePositive(
  primaryIssue: string,
  combinedText: string,
): boolean {
  if (primaryIssue !== 'rate') return false;
  if (!combinedText) return false;
  const lower = combinedText.toLowerCase();
  // If any strong financial trigger is present → not a false positive
  if (FINANCIAL_STRONG_TRIGGERS.some(t => lower.includes(t))) return false;
  // Check if "rate" appears only as substring of non-financial words
  const hasRateSubstring = RATE_SUBSTRING_FALSE_POSITIVE_WORDS.some(w => lower.includes(w));
  // And "rate" as a standalone word is NOT present
  const hasRateStandalone = /\brate\b/.test(lower);
  return hasRateSubstring && !hasRateStandalone;
}

// ─── Provided Doc Misclassification Detection ────────────────────

/**
 * Returns true if the text has provided-doc signals but is classified as
 * a missing-doc category (customs, t1, portbase, bl).
 *
 * This detects false positives where "please find attached T1" was classified
 * as t1 (missing) instead of ref_provided.
 *
 * @example
 * validateProvidedDocMisclassified('t1', 'please find attached the T1 document')
 * // → true
 *
 * validateProvidedDocMisclassified('t1', 'T1 still missing please send')
 * // → false
 */
const PROVIDED_DOC_TEXT_PATTERNS: string[] = [
  ...
  't1 in bijlage', 'douane in bijlage', 'i have attached', 'we have attached',
  'please find the t1', 'mrn below', 't1 number below',
  'find attached mrn', 'please find attached', 'herewith',
];

export function validateProvidedDocMisclassified(
  primaryIssue: string,
  combinedText: string,
): boolean {
  const MISSING_DOC_TOPICS = new Set(['customs', 't1', 'portbase', 'bl']);
  if (!MISSING_DOC_TOPICS.has(primaryIssue)) return false;
  if (!combinedText) return false;
  const lower = combinedText.toLowerCase();
  return PROVIDED_DOC_TEXT_PATTERNS.some(p => lower.includes(p));
}

// ─── Equipment-as-RefProvided Detection ──────────────────────────

/**
 * Returns true if the text has equipment signals but is classified as ref_provided.
 * Equipment condition failures must never be classified as Reference Update.
 *
 * @example
 * validateEquipmentAsRefProvided('ref_provided', 'container damaged ref: XYZ')
 * // → true (should be equipment, not ref_provided)
 *
 * validateEquipmentAsRefProvided('equipment', 'container damaged ref: XYZ')
 * // → false (correctly classified)
 */
const EQUIPMENT_AS_REF_SIGNALS: string[] = [
  'portable not ok', 'container damaged', 'equipment issue', 'container not ok',
  'container defect', 'reefer defect', 'equipment failure', 'equipment fault',
  'container fault', 'trailer defect', 'trailer fault', 'unit not ok',
  'equipment not ok', 'container beschadigd', 'container beschaedigt',
  'container defekt', 'schaden am container',
];

export function validateEquipmentAsRefProvided(
  primaryIssue: string,
  combinedText: string,
): boolean {
  if (primaryIssue !== 'ref_provided') return false;
  if (!combinedText) return false;
  const lower = combinedText.toLowerCase();
  return EQUIPMENT_AS_REF_SIGNALS.some(p => lower.includes(p));
}
