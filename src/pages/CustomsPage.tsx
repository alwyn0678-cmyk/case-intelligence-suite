...
export function CustomsPage({ analysis }: Props) {
  ...
  return (
    <div className="p-8 space-y-8">
      <SectionHeader title="Customs & Compliance" subtitle="Portbase, B/L, T1, and documentation issue patterns" />
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label: 'Customs Docs',            value: cc.customsDocs, accent: '#d8a34c' },
          ...
          {cc.t1Issues > 0 && (
            <div className="bg-[#8b7cff]/8 border border-[#8b7cff]/20 rounded-lg px-4 py-3">
              <p className="text-sm text-[#eceff7]">
                <span className="text-[#8b7cff] font-semibold">{cc.t1Issues}</span> T1 / transit document cases detected — review pre-departure transit declaration process.
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-[#171922] border border-[#2a2f3f] rounded-lg p-5">
              <p className="text-sm font-medium text-[#eceff7] mb-4">Compliance Breakdown</p>
              {donutData.length > 0
            ...
            <div className="bg-[#171922] border border-[#2a2f3f] rounded-lg p-5">
              <p className="text-sm font-medium text-[#eceff7] mb-4">Top Compliance Offenders</p>
              <div className="space-y-2">
                {cc.topOffenders.slice(0, 10).map((o, i) => (
                  <div key={o.name} className="flex items-center gap-3">
                    <span className="text-xs text-[#a6aec4] w-5 text-right">{i + 1}</span>
                    <div className="flex-1 bg-[#2a2f3f] rounded-full h-1.5">
                      <div className="h-1.5 rounded-full bg-[#d8a34c]"
                        style={{ width: `${cc.topOffenders[0] ? (o.count / cc.topOffenders[0].count) * 100 : 0}%` }} />
                    </div>
                    <span className="text-sm text-[#eceff7] w-32 truncate">{o.name}</span>
                    <span className="text-sm text-[#d8a34c] font-medium w-8 text-right">{o.count}</span>
                  </div>
                ))}
              </div>
            </div>
